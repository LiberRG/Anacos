<div class="">
    <div class="h-[45vh] md:h-[89.4vh] relative">
        ...
        <div>
            ...
            <div id="s" x-data="{mostrar:false }" class="absolute font-semibold" @mouseleave="mostrar = false" style="{{ $style }}">
                <div id="{{ $sitio->getId() }}" class="absolute text-magenta-oscuro hover:scale-150" @mouseover="mostrar = true">
                ...
                md:h-auto md:w-80 p-5 md:p-2 m-3 md:m-0 rounded-lg bg-blanco z-[100]">
                    ...
                    <div class="flex flex-col items-center justify-between gap-3 md:gap-2">
                        ...
                        <p class="p  text-sm">{{ $sitio->getDescripcion() }}</p>
                    </div>
                </div>
                <div x-cloak x-bind:class="mostrar ? 'inline' : 'hidden'" class="w-full fixed inset-0 z-[90] bg-negro/[0.7] md:hidden" @click="mostrar= false">
                </div>
            </div>
            @endforeach
        </div>
